...
/// <summary>
/// Provides warm (pre-connected) live streams for fast LiveTV channel zapping.
...
/// <remarks>
/// <para>
/// <b>Adoption flow (Phase 3b):</b> When <c>MediaSourceManager.CloseLiveStream</c>
/// is about to close a stream (ConsumerCount reaches 0), it offers the stream to
/// registered <see cref="IWarmStreamProvider"/> instances. If a provider adopts it,
/// the stream stays alive in <c>_openStreams</c> with ConsumerCount bumped to 1.
/// </para>
/// <para>
/// <b>Reuse flow (Phase 3c):</b> When a new client opens the same channel,
/// <c>DefaultLiveTvService.GetChannelStreamWithDirectStreamProvider</c> searches
/// <c>currentLiveStreams</c> by <c>OriginalStreamId</c>. Since the warm-adopted
/// stream is still in <c>_openStreams</c>, it is found and reused automatically
/// via existing stream sharing (no explicit warm lookup hook required).
/// </para>
/// <para>
/// <b>Eviction:</b> The provider is responsible for eventually calling
/// <c>IMediaSourceManager.CloseLiveStream(id)</c> when evicting a warm stream
...
{
    /// <summary>
    /// Checks if a warm (pre-connected) live stream exists for the given media source.
    /// </summary>
    /// <param name="mediaSourceId">The media source ID (channel identity).</param>
    /// <param name="liveStream">The warm live stream if available.</param>
    /// <returns>True if a warm stream is available for the given media source.</returns>
    bool TryGetWarmStream(string mediaSourceId, out ILiveStream? liveStream);

    /// <summary>
    /// Offers a live stream to the warm pool when its consumer count reaches zero
